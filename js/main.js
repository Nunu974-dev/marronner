  ...
  if (signupFormStep2) {
    ...
    signupFormStep2.addEventListener('submit', async (e) => {
      ...
      if (!firstName || !lastName || !email || !password || !passwordConfirm || !userType) {
        showToast('error', 'Champs manquants', 'Tous les champs obligatoires doivent être remplis.');
        return;
      }
      
      if (password !== passwordConfirm) {
        showToast('error', 'Mots de passe différents', 'Les deux mots de passe doivent être identiques.');
        return;
      }
      ...
      try {
        ...
        if (result.success) {
          ...
          showToast('success', 'Compte créé !', 'Vérifiez votre email pour confirmer votre compte. Redirection...');
          ...
          showToast('error', 'Erreur d\'inscription', result.error || 'Une erreur est survenue. Vérifiez votre connexion.');
    ...
    <div class="modal-form">
      ...
      <div class="signup-step-1">
        ...
        <div class="social-buttons">
          <button class="social-btn" onclick="showToast('info', 'Bientôt disponible', 'La connexion avec Google sera bientôt disponible !')">
          ...
          <button class="social-btn" onclick="showToast('info', 'Bientôt disponible', 'La connexion avec Apple sera bientôt disponible !')">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.24 2.31-.93 3.57-.84 1.51.12 2.65.72 3.4 1.8-3.12 1.87-2.38 5.98.48 7.13-.57 1.5-1.31 2.99-2.54 4.09l.01-.01zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z"/></svg>
            Continuer avec Apple
          </button>
          
          <button class="social-btn" onclick="showToast('info', 'Bientôt disponible', 'La connexion avec Facebook sera bientôt disponible !')">
